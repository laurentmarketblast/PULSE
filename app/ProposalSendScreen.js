...
import {
  ...
  ActivityIndicator, StatusBar, Alert, Keyboard, Platform,
} from "react-native";
...
export default function ProposalSendScreen({ person, onBack, onSent }) {
  ...
  // Slide screen in on mount
  useEffect(() => {
    Animated.parallel([
      Animated.spring(slideAnim, { toValue: 0, friction: 8, tension: 60, useNativeDriver: true }),
      Animated.timing(fadeAnim,  { toValue: 1, duration: 400, useNativeDriver: true }),
    ]).start(() => {
      // Fade in input after screen lands
      Animated.timing(inputOpacity, { toValue: 1, duration: 300, useNativeDriver: false }).start();
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const onShow = () => {
      ...
      Animated.parallel([
        ...
        Animated.timing(nameTop,     { toValue: PHOTO_HEIGHT_COMPACT - 60, duration, useNativeDriver: false }),
      ]).start();
    };
  ...
  return (
    <Animated.View style={[s.root, { transform: [{ translateY: slideAnim }] }]}>
      ...
      {/* Photo — shrinks when keyboard opens */}
      ...
      {/* Name — scales down and repositions */}
      ...
      {/* Input section — always visible below photo */}
...
const s = StyleSheet.create({
  ...
  // Input section — sits below photo in normal flow
  inputSection: {
