...
export default function InboxScreen() {
  ...
  const [activeChat, setActiveChat] = useState(null); // { proposal, otherUser }

  const fetchAll = useCallback(async () => {
    try {
      const [inbox, outbox] = await Promise.all([
        api.getInbox(user.id),
        api.getSent(user.id),
      ]);
  ...
  const handleRespond = async (proposalId, accept) => {
    try {
      const result = await api.respondToProposal(proposalId, accept);
      if (result.id) {
  ...
  const handleOpenChat = (proposal) => {
    ...
    // We don't have full user objects yet — pass what we have
    setActiveChat({
      proposal,
      otherUser: { id: otherUserId, display_name: isReceived ? "Them" : "Them" },
    });
  };
